...
- **New store screenshot: the growth ladder.** A designed slide showing
  all five sprite forms (base gold → cannon → battle armor → jeep →
  tank) with their crate milestones — "Browse websites. Watch your
  rabbit GROW." Generated by `npm run assets` from the stage page's
  `growth` variant. The Web Store caps listings at 5 screenshots, so
  pick 5 of the 6 at upload.
- The slide's base-form sprite gets cannon-style eyes painted on
  (`marketing/stage/gen-front-face.mjs`) — the sheet's front strip is
  faceless; gameplay sprites are untouched.
- Housekeeping: every tracked `.DS_Store` is untracked for real this
  time.
